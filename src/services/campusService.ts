...
export async function createMaterial(
  data: Omit<Material, 'id' | 'createdAt' | 'downloadUrl' | 'storagePath'> & { file: File }
) {
  const { file, ...metadata } = data;
  try {
    if (!file || file.size <= 0) throw new Error('Please select a valid file.');

    // Firebase Storage is intentionally not used. On the free Firebase Spark plan,
    // small PDFs are stored directly in Firestore as a data URL so the material
    // remains available to every signed-in user. Firestore documents are limited
    // to 1 MiB, so keep the source file safely below that limit.
    if (file.type !== 'application/pdf' && !file.name.toLowerCase().endsWith('.pdf')) {
      throw new Error('Only PDF files can be published without Firebase Storage.');
    }
    const maxBytes = 700 * 1024;
    if (file.size > maxBytes) {
      throw new Error('PDF is too large for free Firebase mode. Maximum size is 700 KB.');
    }
    ...
    const dataUrl = await new Promise<string>((resolve, reject) => {
      ...
      reader.onload = () => {
        ...
        else reject(new Error('Could not read the PDF.'));
      };
